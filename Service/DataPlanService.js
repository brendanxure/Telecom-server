...
const CreateDataPlan = async ( network, planName, planId, volume, unit, validity, type, amount ) => {
    try {
        const dataPlan = await DataPlan.create({ network, planName,  planId, volume, unit, validity, type, amount })
...
const BuyData = async( network, planName, planId, volume, unit, validity, type, amount ) => {
    try {
        const dataPlan = await DataPlan.create({ network, planName, planId, volume, unit, validity, type, amount })
        return {code: responsecodes.SUCCESS, success: true, message: 'DataPlan Created Successfully', data: dataPlan}
    } catch (error) {
